    ...
    internal class RPCSUB : MonoBehaviourPunCallbacks
    {
        ...
        public void OnEvent(EventData ev)
        {
            ...
            if (ev.Code == 50)
            {
                object[] arr = (object[])ev.CustomData;
                string customdata = "";
                customdata = $"{arr[2].ToString()} was reported by {arr[3].ToString()} for {MainManager.Instance.getButtonType(int.Parse(arr[3].ToString()))}";
                ShowConsole.Log(customdata);
                Notif.SendNotification($"{arr[2].ToString()} was reported by {arr[3].ToString()} for {MainManager.Instance.getButtonType(int.Parse(arr[3].ToString()))}");
            }
        }
